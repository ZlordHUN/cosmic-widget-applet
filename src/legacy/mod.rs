...
//! Legacy Cairo desktop overlay.
//!
//! This is the retained Cairo implementation of the desktop widget. Unlike the
...
//! # Binary
//!
//! Built by the legacy `cosmic-widget` Cargo target; normal installation uses
//! the production Iced overlay instead.
//! Launch this implementation explicitly with `cargo run --bin cosmic-widget`.
//!
//! # Architecture
//!
...
use self::drawing::load_weather_font;
use self::layout::calculate_widget_height_with_all;
use self::renderer::{RenderParams, render_widget};
use self::theme::CosmicTheme;
use crate::config::{Config, UPDATE_INTERVAL_MS};
use crate::monitors::{
    battery::BatteryMonitor, media::MediaMonitor, network::NetworkMonitor,
    notifications::NotificationMonitor, storage::StorageMonitor, temperature::TemperatureMonitor,
    utilization::UtilizationMonitor, weather::WeatherMonitor,
};
...
struct MonitorWidget {
    ...
    grouped_notifications: Vec<(String, Vec<crate::monitors::notifications::Notification>)>,
...
impl MonitorWidget {
    ...
    fn update_notification_groups(&mut self) {
        ...
        if new_version != self.notifications_version {
            ...
            let mut grouped: HashMap<String, Vec<crate::monitors::notifications::Notification>> =
                HashMap::new();
