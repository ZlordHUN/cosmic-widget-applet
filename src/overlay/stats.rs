...
use crate::monitors::ai_usage::UsageSnapshot;
use crate::monitors::ai_usage::claude::ClaudeUsageMonitor;
use crate::monitors::ai_usage::codex::CodexUsageMonitor;
use crate::monitors::battery::{BatteryDevice, BatteryMonitor};
use crate::monitors::disk_io::DiskIoMonitor;
use crate::monitors::media::{MediaMonitor, MultiPlayerState, PlayerId};
use crate::monitors::network::NetworkMonitor;
use crate::monitors::notifications::{Notification, NotificationMonitor};
use crate::monitors::storage::{DiskInfo, StorageMonitor};
use crate::monitors::temperature::TemperatureMonitor;
use crate::monitors::utilization::UtilizationMonitor;
use crate::monitors::weather::{WeatherData, WeatherMonitor};
...
pub struct SystemSnapshot {
    ...
    pub codex_usage: UsageSnapshot,
    pub claude_usage: UsageSnapshot,
}
...
pub struct StatsSampler {
    ...
    codex_usage_monitor: CodexUsageMonitor,
...
impl StatsSampler {
    pub fn spawn(
        ...
        let sampler = Self {
            ...
            codex_usage_monitor: CodexUsageMonitor::new(ai_usage_enabled),
            claude_usage_monitor: ClaudeUsageMonitor::new(ai_usage_enabled),
        };
        ...
        std::thread::spawn(move || {
            ...
            loop {
                ...
                let snapshot = SystemSnapshot {
                    ...
                    codex_usage: UsageSnapshot::default(),
                    claude_usage: UsageSnapshot::default(),
                };
    ...
    pub fn snapshot(&self) -> SystemSnapshot {
        ...
        snapshot.codex_usage = self.codex_usage_monitor.snapshot();
        snapshot.claude_usage = self.claude_usage_monitor.snapshot();
        snapshot
    }
    ...
    pub fn set_ai_usage_enabled(&self, enabled: bool) {
        self.codex_usage_monitor.set_enabled(enabled);
        self.claude_usage_monitor.set_enabled(enabled);
    }
    ...
    pub fn dismiss_notification(
        &self,
        identity: &crate::monitors::notifications::NotificationIdentity,
    ) {
